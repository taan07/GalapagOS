...
export function buildCompletionDebrief(input: {
  ...
  const { workerId, laneName, retirement } = input;
  ...
  const seed =
    ...
    `Worker ${workerId} (lane "${laneName}") finished its task and the completion PASSED the quality gate (digest status manager_reviewed). Verification and retirement are separate facts. ${retirementFact}\n\n` +
